...
contract Usb is ERC20, IERC865, Ownable {
    ...
    function transferPreSigned(
        ...
        bytes32 hashedTx = keccak256(
            abi.encodePacked(from, ethSignedMessageHash)
        );
        require(
    ...
    function transferFromPreSigned(
        ...
        bytes32 hashedTx = keccak256(
            abi.encodePacked(spender, ethSignedMessageHash)
        );
        require(
            hashedTxs[hashedTx] == false,
            "Transaction hash was already used"
    ...
    function validate(
        ...
        uint256 _fee,
        uint256 _nonce
    ) public view returns (bytes32, address, bytes32) {
        bytes32 hashedParams = getTransferPreSignedHash(
            address(this),
            _to,
            _value,
